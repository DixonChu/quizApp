package uk.ac.aber.dcs.cs31620.quizapp.ui.teacher.update.questions
...
class UpdateQuestions : Fragment() {
    ...
    private fun updateItem() {
        ...
        if (inputCheck(questionName, questionAnswer, optionAns1)) {
            ...
            if ((questionAnswer.toInt() < 0) || (questionAnswer.toInt() > 10)) {
                Toast.makeText(requireContext(), "Please input only 1 to 10 for the correct answer field", Toast.LENGTH_LONG).show()
            } else {
                Toast.makeText(requireContext(), "Please fill in Question Name and Correct answer fields", Toast.LENGTH_SHORT).show()
            }
        }
    }

    private fun inputCheck(questionName: String, questionAnswer: String, optionAns1: String): Boolean {
        return !(TextUtils.isEmpty(questionName)) && (questionAnswer.toInt() > 0) && (questionAnswer.toInt() < 10) && !(TextUtils.isEmpty(optionAns1))
    }
    ...
    private fun deleteQuestion() {
        ...
        builder.setPositiveButton("Yes") { _, _ ->
            ...
            Toast.makeText(
                requireContext(),
                "Successfully removed: ${args.currentQuestion.question}",
                Toast.LENGTH_LONG
            ).show()
            findNavController().popBackStack()
        }
